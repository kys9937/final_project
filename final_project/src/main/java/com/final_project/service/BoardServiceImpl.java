...
import com.final_project.mapper.BoardMapper;
import com.final_project.mapper.FileMapper;
...
public class BoardServiceImpl implements BoardService{
	...
	public boolean regist(BoardDTO board, MultipartFile[] files) throws Exception {
		...
		if(files == null || files.length == 0) {
			...
			for(int i=0;i<files.length-1;i++) {
				...
				fdto.setUb_orgname(orgname);
				fdto.setUb_systemname(systemname);
				if(flag = fmapper.insertFile(fdto) == 1) {
					System.out.println("파일 db에 저장 완료");
				}else {
					System.out.println("파일 db저장 실패");
				}
	...
	public List<FileDTO> getFiles(long boardnum) {
	    logger.info("getFiles 메서드 호출 - boardnum: " + boardnum);
	    List<FileDTO> files = fmapper.getFiles(boardnum);
	    logger.info("getFiles 메서드 실행 완료 - 결과: " + files);
	    for(FileDTO file : files) {
	        logger.info("FileDTO: " + file.toString());
	    }
	    return files;
	}
	...
	public boolean remove(long boardnum) {
		if(bmapper.deleteBoard(boardnum) == 1) {
			...
			for(FileDTO fdto : files) {
				File file = new File(saveFolder,fdto.getUb_systemname());
				if(file.exists()) {
					file.delete();
					fmapper.deleteFileBySystemname(fdto.getUb_systemname());
				}
			}
	...
	@Override
	//loginUser가 작성한 개시글의 총 합 구하기
	public long getUserTotal(String loginUser) {
		return bmapper.getUserTotal(loginUser);
	}
